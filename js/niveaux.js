...
// Chaque niveau a sa forêt ; les étoiles gagnées restent gardées dans chacune.

(function () {
  const $ = id => document.getElementById(id);
  const P = RM.progression;

  // Les étoiles gagnées et possibles dans la forêt d'un niveau
  RM.etoilesNiveau = function (profil, niveau) {
    const chemins = RM.FORETS[niveau].filter(zone => !zone.bientot);
    return {
  ...
  RM.ecrans.niveau = function () {
    ...
    $('grille-niveaux').innerHTML = RM.NIVEAUX.map(niveau => {
      const { gagnees, total } = RM.etoilesNiveau(profil, niveau.id);
      ...
      return `
        <button class="carte-niveau${estActuel ? ' actuel' : ''}" data-niveau="${niveau.id}">
          ...
          <span class="niveau-progres"><span class="etoile gagnee">★</span> ${gagnees} / ${total}</span>
